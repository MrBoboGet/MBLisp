...
(defmethod convert-expr ((expr Expr_Lambda))
    `(lambda (,@(map convert-idf :Variables expr) ) ,@(map convert-statement :Body expr))
)
...
(defmethod / (val (func function_t))
    (func val)
)
(defmethod / (val (func lambda_t))
    (func val)
)
(defmethod / (val (func generic_t))
    (func val)
)
(defmethod / ((val list_t) (func function_t))
    (map func val)
)
(defmethod / ((val list_t) (func lambda_t))
    (map func val)
)
(defmethod / ((val list_t) (func generic_t))
    (map func val)
)
...
(defun parse-expr (stream) 
    (setl current-tokenizer (get-tokenizer))
    (set-stream current-tokenizer stream)
    (setl result (ParseExpr current-tokenizer))
    (convert-expr result)
)
