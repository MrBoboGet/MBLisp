...
(defmacro defmethod (methodname overrides &rest body &envir envir)
  (setl type-overrides (list))
  (setl argument-symbols (list))
  (setl i 0)
  (while (< i (len overrides))
    (setl current-argument (index overrides i))
    (cond (eq current-argument '&rest) (progn (setl i (plus i 2)) (continue)) null)
...
(defun stream-reader (stream)
    ...
    (if (in sym load-envir)
        (setl reader (index load-envir sym))
