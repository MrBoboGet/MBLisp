...
(defun handle-initialize (arg)
 ...
 (if (&& (in "linesStartAt1" arg) (. "linesStartAt1" arg))
    (set line-offset -1)    
 )
 (if (&& (in "columnsStartAt1" arg) (. "columnsStartAt1" arg))
    (set col-offset -1)    
 )
 return-value
)
...
(defun get-request-response (request)
  (set return-value (make-dict))
  (set command (index return-value "command"))
  (set (index return-value "request_seq") (. request "seq"))
  (set (index return-value "seq") (get-seq))
  (set (index return-value "command") (. request "command"))
  (if (not (in (. request "command") request-handlers))
    (set (index return-value "success") false)
    (set (index return-value "body") (make-dict ("error" "request not implemented")))
   else
    (try
    (
      (set (index return-value "body") ((. request-handlers command) (. request "arguments")))
      (set (index return-value "success") true)
    )
    catch (string_t e)
    (
      (set (index return-value "success") false)
      (set (index return-value "body") {error: e})
    )
    catch (any_t e)
    (
      (set (index return-value "success") false)
      (set (index return-value "body") {error: "internal error"})
    )
    )
  )
  return-value
...
(defun main ()
    ...
    (while true
        ...
        (if (eq (. next-message "type") "request")
          (js:send-rpc (get-request-response next-message))
          (if (eq (. next-message "command") "initialize")
            (js:send-rpc {seq: (get-seq), type: "event", event: "initialized"})
          )
        )
    )
)
